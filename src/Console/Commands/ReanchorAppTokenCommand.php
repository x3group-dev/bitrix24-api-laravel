 ...
 *
 * ПЯТЬ КОРЗИН. Различать их обязательно, и по той же причине, по которой их различает
 ...
 *  - ЖДЁТ ВХОДА АДМИНА: про владельца известно, что он не админ, админы на портале есть,
 *    но ни у одного токен не свежее --max-age. Переустановка тут ни при чём: достаточно,
 *    чтобы любой администратор один раз открыл приложение — b24_users обновится, и
 *    следующий прогон портал починит.
 ...
 *  - НЕТ ДАННЫХ: свидетельств о правах владельца нет вовсе. Это ДВА случая, а не один:
 *    либо владелец записан, но строки о нём в b24_users нет, либо владельца нет (user_id
 *    IS NULL) и админов на портале тоже нет. Второй случай — основной у приложений на
 *    API-роутах, ради которых корзина и заведена. Ни трогать такой портал нечего, ни звать
 *    клиента на переустановку не за что. Только считаем.
 *
 * --LIMIT ОГРАНИЧИВАЕТ РЕМОНТ, А НЕ ЗНАНИЕ. Бюджет прогона тратит только корзина ремонта:
 * остальные три не делают ни одного REST-вызова и ничего не пишут. Считаются они поэтому
 * целиком, и обрезаются лишь СТРОКИ ТАБЛИЦЫ — иначе отчёт вырос бы до размера флота.
 * Обрезанный счётчик врал бы ровно там, где цена вранья максимальна: оператор, запустивший
 * дефолтный --limit=200 сразу после миграции по флоту в 25 тысяч порталов, прочитал бы
 * «на переустановку: 200» как полную картину.
 *
...
class ReanchorAppTokenCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        // --limit ограничивает РЕМОНТ, а не знание. Диагностические корзины не делают ни
        // одного REST-вызова и ничего не пишут — бюджет прогона их не касается, поэтому
        // считаются они целиком, и только СТРОКИ ТАБЛИЦЫ обрезаются, чтобы отчёт не вырос
        // до размера флота.
        //
        // Врать здесь дороже всего ровно тогда, когда отчёт нужнее всего: оператор,
        // запустивший дефолтный --limit=200 сразу после миграции по флоту в 25 тысяч
        // порталов, прочитает «на переустановку: 200» как полную картину и закроет вопрос.
        ...
        // Расхождение между строками таблицы и счётчиками сводки обязано быть названо
        // вслух: необъяснённое, оно читается как ошибка отчёта, и оператор поверит той
        // половине, которая короче.
