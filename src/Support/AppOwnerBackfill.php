 ...
 * оставшееся и не пересматривает то, что проставил ремонт. Порталы, закреплённые за
 * системным пользователем приложения (is_system_user), пропускаются.
 */
class AppOwnerBackfill
{
    ...
    public function run(): array
    {
        ...
        // Закреплённый за системным пользователем портал владельца не ищет: он уже
        // проставлен приёмом события. Условие на NULL — страховка: колонка NOT NULL.
        DB::table('b24_apps')
            ->whereNull('user_id')
