 ...
 *
 * Не-админ во владельцах означает не-админский app-токен на портале: админ-методы
 * (userfieldconfig.*) падают «нет прав», а переустановка админом помогает лишь до
 * следующей установки сотрудником.
 *
...
class InstallAdminGuardTest extends TestCase
{
    ...
    /**
     * Профиль без ID — тоже отказ: установка с user_id = NULL оставила бы портал в
     * состоянии, при котором правило 2 не сработает никогда.
     */
    #[DataProvider('installPaths')]
     ...
     *
     * Storage-правило разрешает первую установку кем угодно (строки ещё нет — затирать
     * нечего), поэтому защитой от не-админского владельца оно НЕ является: без гейта выше
     * сотрудник спокойно становится владельцем свежего портала.
     */
