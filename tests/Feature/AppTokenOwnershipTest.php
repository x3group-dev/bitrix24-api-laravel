...
class AppTokenOwnershipTest extends TestCase
{
    ...
    /**
     * Владелец не должен вычисляться из ВХОДЯЩЕГО токена.
     *
    ...
    /**
     * Владелец не должен вычисляться и из УЖЕ ЛЕЖАЩЕГО в b24_apps токена.
     *
    ...
    /**
     * Отказ обязан быть виден в логе: молчаливое правило 2 скрыло бы и подмену
     * app-токена, и законный рефреш, заблокированный протухшим user_id.
     */
     ...
     *
     * Это безопасный fail-closed отказ, целиком читаемый одним запросом к колонке
     * (where user_id is null). Громким его делать нельзя из-за объёма: у порталов, где
     * никто не открывал фронтенд, user_id остаётся NULL, и notice здесь утопил бы в себе
     * единственный настоящий сигнал.
     */
    ...
    /**
     * Две причины отказа обязаны различаться в логе: «портал захватывает чужой» — это
     * инцидент, а «владелец не установлен» — ожидаемое состояние части флота. Точные
     * формулировки не фиксируем — важно, что они не совпадают.
     */
    ...
    {
        // Портала нет — это не отказ, а обычный no-op (приложение уже удалено).
        $this->writer()->propagateFromUser('member-absent', self::INSTALLER, $this->token('fresh'));
    ...
    /**
     * Рефреш владельца обязан обновлять ОБЕ строки, иначе app-токен портала протухнет
     * сам по себе.
     */
    ...
    /**
     * Зеркало предыдущего: рядовой сотрудник обновляет ТОЛЬКО себя. Проверяется
     * содержимое колонки портала, а не отсутствие исключения — неправильная запись была
     * бы тихой.
     *
     * Единственный сценарий, где видно, что обновляющегося пользователя берут из
     * хранилища, а не из b24_apps.user_id: портал принадлежит 221, рефреш идёт от 162, и
     * тавтологическая реализация увидела бы 221 === 221 и записала бы 'employee'.
     */
    ...
    /**
     * Пара (member_id, user_id) собирается из тех же источников, по которым ищется строка
     * пользователя: member_id — из ОБНОВЛЁННОГО ТОКЕНА, user_id — из хранилища. Рядом в
     * saveRenewedToken лежат и $this->memberId, и $renewedAuthToken->memberId, а подмена
     * одного другим означала бы тихую запись в чужой портал.
     *
    ...
    /**
     * Заход рядового сотрудника: своя строка обновляется, портал — нет. Основной путь,
     * которым чужой токен попадал в b24_apps: он срабатывает на каждом открытии
     * приложения, а не только на рефреше по истечении срока.
     */
     ...
     *
     * Исполнитель принимает int, и пользователь 0 для него — обычный не-владелец, то есть
     * notice «обновляет не владелец». Этот notice — единственный громкий сигнал фичи, и
     * означать он должен попытку захвата портала, а не «профиль пришёл без ID».
     */
     ...
     * 401 на всё размещение, причём владельцу. Отсюда гейт выше по течению.
     */
