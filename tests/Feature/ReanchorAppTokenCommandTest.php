 ...
 *
 * Проверяются четыре свойства команды:
 *
 *  - ремонт означает ЗАПИСЬ ТОКЕНА, а не только колонки user_id;
 *  - трогается РОВНО ТОТ портал и берётся РОВНО ЕГО админ (во всех фикстурах токены
 *    именованы через member_id: «m-a/user-10-access» показывает и чей токен, и с какого
 *    он портала);
...
class ReanchorAppTokenCommandTest extends TestCase
{
     ...
     *
     * Каждая колонка обязана отличаться от того, что придёт из b24_users: совпадающее
     * значение сделало бы забытую при откате колонку невидимой.
     */
    ...
    /**
     * Проверка подменена НА УСПЕШНУЮ намеренно: при неуспешной реализация, пишущая и на
     * dry-run, сама же откатила бы строку, и лишняя запись осталась бы невидимой.
     */
     ...
     * пишет now() + expires, то есть сложил бы два timestamp-а.
     */
     ...
     *
     * Выбор админа без фильтра по member_id записал бы в m-target токен портала
     * m-foreign. Тот же тест ловит потерю корреляции в проверке владельца: без неё
     * m-foreign (владелец 10, админ у себя) увидел бы чужую строку и поехал бы в ремонт.
     */
     ...
     *
     * Это норма для целого класса приложений: is_admin заполняет только
     * B24AppUserMiddleware, поэтому у приложения на routes/b24app.php «нет строки про
     * владельца» — состояние всего флота. Ту же границу проводит корзина unknownOwner в
     * {@see \X3Group\Bitrix24\Support\AppOwnerBackfill}.
     *
     ...
     * сломанным.
     *
     * Нормальное состояние приложения на routes/b24app.php: после бэкофилла его порталы
     * делятся на «user_id остался NULL» и «user_id заполнен со старой установки», и оба
     * вида здоровы.
     *
     * Тест закрывает соблазн описать «нужна переустановка» как «владелец не подтверждён
     * админом И админов нет»: такая формулировка отправила бы туда весь флот. Условие
     * построено от ПОЛОЖИТЕЛЬНОГО свидетельства — владелец найден в b24_users и там
    ...
    /**
     * Гейт админства. Команда пишет в b24_apps сама, поэтому AppTokenWriter::shouldWrite
     * вызывается явно: без него опечатка в --user сделала бы владельцем не-админа.
     *
     ...
     *
     * У сломанного портала токен рядового сотрудника обычно ЖИВОЙ: работают все методы,
     * кроме админских. Перепривязка на давно протухшего админа убила бы портал целиком.
     */
    public function test_a_stale_admin_is_not_used_as_a_new_anchor(): void
    {
        ...
        // Портал обязан быть ВНЕ корзины ремонта: попав в неё, он съедал бы бюджет
        // каждого прогона.
     ...
     * есть в корзине ремонта забирал бы --limit на каждом прогоне. Поэтому проверяется не
     ...
     *
     * Гонка не гипотетическая: живой вызов идёт через диспетчер appEvents, слушатель
     * которого сохраняет обновлённый токен прямо в b24_apps, и слепой откат вернул бы
     * туда уже отозванное Битриксом значение.
     */
    public function test_rollback_is_cancelled_when_the_row_changed_underneath(): void
    {
        ...
        // А вот владелец обязан вернуться: оставить в user_id подтверждённого не-админа
        // значило бы закрепить за порталом исходную аварию (здесь прежний владелец NULL,
        // то есть портал уехал бы из fail-closed в «стабильно ломается»).
        self::assertNull(
            $app->user_id,
    ...
    /**
     * --limit нельзя тратить на строки, которые починить нельзя: порядок стабилен по id,
     * и непочиняемый портал забирал бы бюджет на каждом запуске, а стоящий следом
     * починяемый не чинился бы НИКОГДА.
     */
    ...
    /**
     * --limit ограничивает РЕМОНТ, а не знание. Счётчики диагностических корзин обязаны
     * быть полными.
     *
     * Диагностические корзины не делают REST-вызовов и ничего не пишут — ограничивать их
     * нечего, а обрезанный счётчик оператор прочитал бы как полную картину. Строки
     * таблицы при этом обрезаются, поэтому проверяются обе половины: счётчик полный,
     * таблица короче счётчика и об обрезке сказано вслух.
     */
    public function test_limit_does_not_truncate_the_diagnostic_counts(): void
    {
        // Нужна переустановка: владелец точно не админ, админов на портале нет.
        foreach (['m-stuck-1', 'm-stuck-2', 'm-stuck-3'] as $memberId) {
            $this->portal($memberId, 40);
            $this->user($memberId, 40, isAdmin: false, expires: time() + 3600);
        }

        // Ждут входа админа: админ есть, но протух.
        foreach (['m-stale-1', 'm-stale-2', 'm-stale-3'] as $memberId) {
            $this->portal($memberId, null);
            $this->user($memberId, 10, isAdmin: true, expires: time() - 40 * 86400);
        }
        ...
        self::assertStringContainsString(
            'на переустановку: 3',
            $out,
            'счётчик «нужна переустановка» обрезан по --limit: оператор прочитает часть флота как весь',
        );
        self::assertStringContainsString(
            'ждут входа админа: 3',
            $out,
            'счётчик «ждут входа админа» обрезан по --limit',
        );

        // Таблица при этом обрезана.
        self::assertSame(
            1,
            substr_count($out, 'нет админов в b24_users'),
            'строки диагностической корзины не обрезаны по --limit: отчёт вырастет до размера флота',
        );
        self::assertStringContainsString(
            'показаны не все',
            $out,
            'таблица обрезана молча: расхождение строк со счётчиком выглядит как ошибка отчёта',
        );
    }
     ...
     *
     * Намеренно не через $this->artisan(): его expectsOutputToContain гасит одним
     * выводом ровно одно ожидание — первое подошедшее, из-за чего
     * doesntExpectOutputToContain молча не срабатывает, если ту же строку уже забрало
     * положительное ожидание. Здесь вывод берётся целиком и проверяется ассертами.
     *
    ...
    /**
     * Токены именованы через member_id: одинаковый user_id на разных порталах — норма
     * флота, и без префикса запись ЧУЖОГО токена была бы неотличима от правильной.
     */
