...
class ReanchorAppTokenCommandTest extends TestCase
{
     ...
     * быть полными.
     *
     * Это не косметика отчёта. Диагностические корзины не делают ни одного REST-вызова и
     * ничего не пишут — ограничивать их нечего, зато цена вранья максимальна ровно в тот
     * момент, когда отчёт нужнее всего: оператор, прогоняющий сразу после миграции
     * дефолтный --limit=200 по флоту в 25 тысяч порталов, прочитает «на переустановку: 200»
     * как полную картину и закроет вопрос. Настоящее число могло быть любым.
     *
     * Строки таблицы при этом обрезаются как раньше: печатать десятки тысяч строк — не
     * отчёт. Поэтому проверяются обе половины: счётчик полный, таблица короче счётчика и
     * об обрезке сказано вслух.
     */
    public function test_limit_does_not_truncate_the_diagnostic_counts(): void
    {
        ...
        // Таблица при этом обрезана — иначе «честный счётчик» куплен ценой отчёта на
        // десятки тысяч строк.
        self::assertSame(
            1,
