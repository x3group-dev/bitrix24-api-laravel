...
Развилка не спрятана внутрь `InstallService`: у install-флоу тип ответа — `view` (страница
...
`GrantSystemUserEntityRightsJob` — она выдаёт системному пользователю право `X` на все
...
Права раздаются **прежним токеном владельца-администратора**: он снимается со строки
...
задачи (через две минуты) грант администратора протух, в лог идёт отдельное сообщение
...
точка расширения для приложения: пакет не знает, что после переезда нужно сделать
конкретно ему (перестроить кеш, уведомить администратора и т.п.).

**Важно:** `bitrix24:reanchor-app-token` и `bitrix24:backfill-app-owner` порталы,
закреплённые за системным пользователем, пропускают — их app-токен перепривязывать нельзя.
