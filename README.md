...
- Регистрируются упорядоченным списком в `config('bitrix24.installers')`:
  ...
  ```
- Пакет (`InstallService` + `AppSetupRunner`) вызывает их **по порядку** и на install-странице, и на
  событии `ONAPPINSTALL` (идемпотентно, из одного места).
- **Установщики ОБЯЗАНЫ быть идемпотентны.** Ошибка любого установщика логируется и пробрасывается
  (fail-loud), install-страница показывает `install-fail` — то есть неидемпотентный установщик (напр.
  «поле уже существует») превратит открытие приложения в постоянный экран ошибки.
- **App-токен портала** (`b24_apps`) пишется только если строки ещё нет (первая установка) **или**
  открывающий — администратор (`profile->ADMIN`); токен не-админа существующий не перезатирает
  (`AppTokenWriter`, анти-угон). По умолчанию `installers` пуст — обратная совместимость.
