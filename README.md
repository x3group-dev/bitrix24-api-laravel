...
Миграция заполняет колонку для установленных порталов: владелец декодируется из `access_token` и
принимается, только если это подтверждённый по `b24_users` администратор. Печатает сводку и список
порталов на ремонт; повторный запуск продолжает с места обрыва.
