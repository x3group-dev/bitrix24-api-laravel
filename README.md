...
- На storage-уровне действует ещё и `AppTokenWriter` (анти-угон): токен не-админа существующую
  строку не перезатирает, а обновлённый токен уезжает в `b24_apps` только от владельца портала.
