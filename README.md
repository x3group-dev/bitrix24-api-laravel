...
Миграция только заводит колонку — заполнение и ремонт вынесены в команды. Порядок выката:
...
### Заполнение владельца: `bitrix24:backfill-app-owner`
...
Проставляет `user_id` порталам, установленным до появления колонки: владелец декодируется из
`access_token` и принимается, **только если это подтверждённый по `b24_users` администратор того же
портала**; любое сомнение оставляет `NULL` (fail-closed). Ничего не чинит — только записывает уже
сложившееся владение.

Обрабатываются строки с `user_id IS NULL`, поэтому прогон безопасно повторять: он дозаполняет то,
про что знание о правах появилось позже (`is_admin` пишется, когда пользователь заходит во
фронтенд), и не пересматривает ни проставленное ранее, ни назначенное вручную ремонтом.

Сводка делит порталы на четыре корзины: **заполнено**; **владелец не админ** — это и есть поломка,
такие порталы перечисляются поимённо и идут в `bitrix24:reanchor-app-token`; **владелец неизвестен**
— строки о нём в `b24_users` нет, для приложений на API-роутах это норма; **токен не разобран** —
единичные случаи нормальны, массовые означают ошибку интеграции.

Опции:

- `--dry-run` — посчитать и показать сводку, не записывая ничего. Заполнение при этом прогоняется
  по-настоящему, но в заведомо откатываемой транзакции, поэтому цифры точные, а не оценочные.
...
Подбирает администратора портала по `b24_users` и перепривязывает портал на него.

**Пишет токен, а не только колонку:** смена одного `user_id` ничего не чинит — правило 2 донесло бы
токен нового владельца лишь когда тот сам откроет приложение. Команда переносит токен админа целиком
и обнуляет `error_update` (при `error_update >= 10` портал выпадает из ротации обновления токенов).
Результат проверяется живым вызовом `user.admin`; исходов три: **перепривязан**, **откат** (строка
возвращена целиком) и **откат отменён** — строку успел переписать кто-то ещё, поэтому токен
оставляется свежий, а владелец возвращается прежний. Прогон без единой выжившей перепривязки
...
корзину «нет данных» закономерно попадает весь флот — звать клиентов на переустановку тут не за что.
...
- `--limit=200` — максимум **ремонтируемых** порталов за прогон, `0` — без ограничения. Счётчики в
  сводке всегда полные, за весь флот; обрезаются только строки таблицы.
- `--max-age=30` — не брать во владельцы админа, чей токен не обновлялся дольше стольких дней (`0` —
  без ограничения). Защищает от замены живого токена не-админа на мёртвый токен админа.
- `--skip-verify` — не проверять результат вызовом `user.admin`. **Разрешён только вместе с
  `--member`:** без живой проверки прогон способен разложить по флоту мёртвые токены.
