...
> **Ловушки, выявленные при ревью T9 — учесть обязательно.**
>
> 1. **Админский гард не путешествует вместе с `user_id`.** Проверка, не дающая не-админу
>    стать владельцем, живёт в `InstallService`, а НЕ в `AppTokenWriter`
>    (`shouldWrite` = `!$appExists || $isAdmin`). Если команда запишет `user_id` напрямую
>    через `DB::table('b24_apps')->update(...)`, как это делает `AppOwnerBackfill`, она не
>    унаследует никакого гарда и сможет заново узаконить не-админа — по всему флоту.
>    Либо проводить запись через `saveIfAllowed(..., $userId)` (там `$appExists` всегда
>    true, значит `$isAdmin` реально работает воротами), либо продублировать проверку
>    со своим тестом.
> 2. **Перепривязка одного `user_id` портал НЕ чинит.** После T8/T9 токен станет верным
>    только когда новый владелец в следующий раз откроет приложение. Если админ живёт
>    исключительно на API-маршрутах, портал так и останется со сломанным не-админским
>    токеном. Команда обязана писать и сам токен, а не только колонку владельца.
> 3. Столкновения с бэкофиллом нет: `AppOwnerBackfill` трогает только `user_id IS NULL`,
>    так что повторный прогон не переедет починенные строки. Это спроектировано, не случайность.
...
> **Накопилось за T7–T10 — включить в README.**
>
> - Правило 2 описано как «только на уровне хранилища», но после T9 основной путь —
>   открытие приложения владельцем (placement). Это надо задокументировать.
> - Перечислить, что искать в логах: `notice` — чужой пытался перезаписать токен портала
>   (единственный громкий сигнал); `debug` — пропуски: владелец не установлен, пользователь
>   не опознан, в placement нет refresh-токена.
> - Описать команду `bitrix24:reanchor-app-token` (второй консольной команды в README нет
>   вовсе), её четыре корзины и то, что «нет данных» ≠ «на переустановку».
> - Явно сказать: перепривязка владельца **не чинит** портал сама по себе — команда пишет
>   и токен; для приложений на API-маршрутах `b24_users` не заполняется, и это норма.
> - Установка: не-админ или неопознанный установщик теперь получает
>   `InstallerCannotOwnPortalException`, установка прерывается, ничего не пишется.
>
> **Долг, вынесенный за рамки ветки (записать как задачу, не делать здесь):** обе точки
> записи в `b24_apps` (`InstallService` и `B24AppUserMiddleware`) покрыты только
> структурными тестами, потому что строят SDK-клиент внутри себя. Выход — вынести получение
> профиля за container-резолвимый сотрудник, тогда `handle()` станет исполнимым и три
> структурных файла схлопнутся до инвариантов, которые действительно нельзя выполнить.
